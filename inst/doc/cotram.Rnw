...
class, as proposed and discussed in \cite{Siegfried_Hothorn_2019}, are 
...
and \pkg{mlt} \citep{Hothorn_2018_JSS,pkg:mlt} for likelihood-based inference and
...
\pZ\left(\h_\rY\left(\lfloor \ry \rfloor\right) - \rx^\top \shiftparm \right),
\quad \ry \in \RR^{+}
\end{eqnarray}
%
with specific models originating from the choice of the different link functions $\pZ^{-1}$. 
...
the transformation function $\h_\rY$ and the regression coefficients $\shiftparm$
...
$\h_\rY$ and interpretation of the linear predictor as discrete hazard ratio.
...
$\h_\rY(\ry)$ cancels out in the odds ratio, resulting in an estimate, which
...
Specifying a count transformation model with log-log link $\pZ^{-1}$ we
get the model formulation
%
\begin{eqnarray*} \label{mod:loglog}
\pYx(\ry \mid \rx) = \Prob(\rY \le \ry \mid \rx)
=  \exp \left(-\exp\left(\h_\rY(\lfloor \ry \rfloor) - \rx^\top \shiftparm \right)\right)
\end{eqnarray*}
%
...
transformed counts $\Ex(\h_\rY(\ry) \mid \rX = \rx) = \rx^\top \shiftparm$. This
...
transformation function $\h_\rY(\ry)$ as illustrated in Figure~\ref{fig:probit-trafo}.
%
\begin{figure}[h]
<<probit_trafo>>=
plot(mod_probit, type = "trafo", newdata = df[1,], smooth = TRUE, 
     ...
     col = col[10], lwd = 2)
@
\caption{Deer-vehicle collisions. Baseline transformation $\h_\rY$ estimated
by the model \code{mod\_probit} illustrated as smoothed line.\label{fig:probit-trafo}}
\end{figure}
%
...
\includepdf[pages=1-,nup=1]{count_transformation}
